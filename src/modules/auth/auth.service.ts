...
export class AuthService {
  ...
  async registerUsers(dto: CreateUserDTO): Promise<AuthUserResponse> {
    ...
    try {
      ...
      this.logger.log(`User data: ${JSON.stringify(dto)}`);
      ...
      const userResponse: UserResponse = {
        ...
        themeModeDevice: user.themeModeDevice,
        popupForNewUser: user.popupForNewUser,
        avatar: user.avatar,
        switcherFolder: user.switcherFolder,
        colors: user.colors
      };

      // return { user, token };
      this.logger.log(`UserResponse: ${JSON.stringify(userResponse)}`);
